...
class PolyglotWriter:
    ...
    def merge_files(input_paths: List[Path], output_path: Path) -> int:
        """
        Merge multiple .bin files into one.
        
        Args:
            input_paths: List of input .bin file paths
            output_path: Output merged .bin file path
            
        Returns:
            Number of entries in merged file
        """
        all_entries = {}
            ...
            try:
                with open(input_path, 'rb') as f:
                    while True:
                        ...
                        pos_hash, encoded_move, weight, _ = struct.unpack('>QHHi', data)
                        key = (pos_hash, encoded_move)
                        
                        if key in all_entries:
                            all_entries[key] = min(65535, all_entries[key] + weight)
                        else:
                            all_entries[key] = weight
            except Exception:
                continue
        ...
        # Sort by hash
        sorted_entries = sorted(all_entries.items(), key=lambda x: x[0][0])
        
        # Write merged file
        output_path.parent.mkdir(parents=True, exist_ok=True)
        
        with open(output_path, 'wb') as f:
            for (pos_hash, encoded_move), weight in sorted_entries:
                entry = struct.pack('>QHHi', pos_hash, encoded_move, weight, 0)
                f.write(entry)
