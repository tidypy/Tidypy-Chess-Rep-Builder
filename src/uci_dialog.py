...
class UCIConfigDialog(QDialog):
    ...
    def _setup_ui(self) -> None:
        ...
        info_layout.addRow("Path:", QLabel(str(self.engine_path.name)))
        ...
        tb_options = ['SyzygyPath', 'NalimovPath', 'GaviotaTbPath']
        tb_found = None
        for tb_opt in tb_options:
            if tb_opt in self.options:
                tb_found = tb_opt
                break
        ...
        if tb_found:
            ...
            line = QLineEdit()
            line.setPlaceholderText(f"Path to {tb_found.replace('Path', '')} tablebases...")
            self.option_widgets[tb_found] = line
            tb_layout.addRow(f"{tb_found}:", line)
            layout.addWidget(tb_group)
        
        # Note about multi-worker
        note_label = QLabel("ℹ️ Hash and Threads will be divided among workers if using multiple workers.")
